...
// Catalog implements `egress-guard catalog fetch`: download, validate, and
// install the baseline catalog at baselineCatalogPath().
func Catalog(args []string) error {
	if len(args) == 0 || args[0] != "fetch" {
		return fmt.Errorf("usage: egress-guard catalog fetch [--url <url>]")
	}
	fs := flag.NewFlagSet("fetch", flag.ContinueOnError)
	url := fs.String("url", DefaultCatalogURL, "catalog URL")
	pubPath := fs.String("pubkey", "", "Ed25519 public key file; verifies <url>.sig before install")
	...
	if *pubPath != "" {
		...
		if err := catalogfetch.Fetch(ctx, *url, dest, catalogfetch.HTTPFetcher{}); err != nil {
			return err
		}
	}
